...
installer() {
  ...
  path=$(get_path)
  target="$path/musicplayer"
  if ! [ -f "musicplayer.sh" ]; then
    curl https://raw.githubusercontent.com/iamaloksy/Musicplayer/main/musicplayer.sh -o musicplayer.sh
  fi

  if [ -n "$path" ]; then
    doas cp musicplayer.sh "$target"
...
final() {
  ...
  target="$path/musicplayer"
  if [ -e "$target" ]; then
    success "Musicplayer has installed successfully."
    success "Run 'musicplayer' to start it!"
  else
    error "Failed to install Musicplayer"
  fi
}
